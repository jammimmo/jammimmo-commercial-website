 ...
 *   • streets   — OpenFreeMap **Liberty** (coloured) vector tiles (MapLibre GL),
 *                 mounted inside Leaflet via `maplibre-gl-leaflet` (see VectorBasemap).
 ...
 * services). We use the **Liberty** style — coloured (green landuse, blue water,
 * coloured roads) but still editorial — with no key to provision. Data ©
 * OpenStreetMap, tiles © OpenMapTiles, hosting by OpenFreeMap. See
 * https://openfreemap.org (other keyless styles: positron = grey, bright = vivid).
 *
...
/**
 * OpenFreeMap Liberty — keyless **coloured** vector street style (MapLibre GL).
 * No env var, no key: this is the production default. Rendered via
 * `maplibre-gl-leaflet` inside the existing Leaflet map (markers/circles stay
 * native Leaflet). Swap the last path segment for another keyless style if
 * needed: `positron` (grey/minimal) or `bright` (vivid).
 */
